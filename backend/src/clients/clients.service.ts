...
export class ClientsService {
  ...
  async create(createClientDto: CreateClientDto) {
    const [client] = await this.db.insert(schema.clients).values(createClientDto).returning();
    return client;
  }
  ...
  async findAll(query: ListClientsDto) {
    ...
    const [totalCount] = await this.db.select({ count: sql<number>`cast(count(*) as int)` }).from(schema.clients);
