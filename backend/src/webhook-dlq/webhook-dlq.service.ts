  ...
  async findAll(query: ListWebhookDlqDto) {
    ...
    const queryBuilder = this.db
      ...
      .innerJoin(
        ...
        eq(schema.webhookDlq.webhookDeliveryId, schema.webhookDeliveries.webhookDeliveryId),
      )
      .innerJoin(
        schema.webhookEvents,
        eq(schema.webhookDeliveries.webhookEventId, schema.webhookEvents.webhookEventId),
      );
    ...
    const data = await queryBuilder.limit(limit).offset(offset).orderBy(desc(schema.webhookDlq.createdAt));
  ...
  async replay(id: string, dto: import("./dto/replay-webhook-dlq.dto").ReplayWebhookDlqDto) {
    ...
    if (existingDelivery) {
      ...
      if (existingDelivery.deliveryStatus === "success") {
        ...
        return {
          status: "Delivery already succeeded",
          deliveryId: existingDelivery.webhookDeliveryId,
        };
      }
    }
